object FrmPedidoWeb: TFrmPedidoWeb
  ...
  object cdsPessoas: TFDQuery
    ...
    SQL.Strings = (
      'select * from pessoa'
      'where'
      '((codigolocal is null) or (codigolocal = 0))'
      'order by razao')
    Left = 432
    Top = 176
  ...
  object qryOrcamento: TFDQuery
    ...
    SQL.Strings = (
        ...
        'select orc.*, emp.cweb, emp.codigo as cdempresa from Orcamento O' +
        'RC'
      'inner join empresa emp on orc.FKEMPRESA = emp.codigo'
      'where orc.CODIGO_WEB = :ID')
    Left = 64
    Top = 56
    ParamData = <
      item
